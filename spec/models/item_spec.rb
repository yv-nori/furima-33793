...
describe Item do
  ...
  describe 'すべての項目' do
    context 'image、name、info、info、category_id、sales_status_id、shipping_fee_status_id、prefecture_id、scheduled_delivery_id、price、sale_user_idが存在する時' do
      let(:item){ build(:item) }
  ...
  describe 'priceの検証' do
    ...
    context '9999999以下の場合' do
      let(:item){ build(:item, price: 9999999) }
      it_behaves_like '登録できる'
    end
    context '299以下の場合' do
    ...
    context '10000000以上の場合' do
      let(:item){ build(:item, price: 10000000) }
      it_behaves_like '登録できない'
    end
  ...
  describe 'sale_user_idの検証' do
    context '空の場合' do
      let(:item){ build(:item, sale_user_id: nil) }
      it_behaves_like '登録できない'
    end
  end
end
